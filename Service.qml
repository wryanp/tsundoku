...
Item {
  ...
  property var openCaps: ({ mpv: false, ytdlp: false })
  ...
  // Routes through Providers.openPlan so watch links land in mpv when it
  // exists on this machine, with a silent browser fallback otherwise.
  // Returns the method used so the IPC open can report it; "" means the id
  // wasn't found.
  ...
  // Probes for mpv and yt-dlp once at startup. Never touches openCaps on
  // a bad parse or a non-zero exit — the all-false default (browser for
  // everything) is always safe.
  Process {
    id: capsProc
    command: ["sh", "-c", "m=false; command -v mpv >/dev/null 2>&1 && m=true; y=false; command -v yt-dlp >/dev/null 2>&1 && y=true; printf '{\"mpv\":%s,\"ytdlp\":%s}\\n' \"$m\" \"$y\""]
